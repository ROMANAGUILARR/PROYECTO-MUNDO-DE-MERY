...
public class ProductService {
    ...
    public ProductDetailResponse getProductDetailsById(Long id) {
        ...
        // coleccion de talla - ordenar por sortOrder y agregar precio
        List<SizeDTO> sizesDTO = product.getSizes().
                stream()
                .sorted((s1, s2) -> {
                    Integer order1 = s1.getSortOrder() != null ? s1.getSortOrder() : Integer.MAX_VALUE;
                    Integer order2 = s2.getSortOrder() != null ? s2.getSortOrder() : Integer.MAX_VALUE;
                    return order1.compareTo(order2);
                })
                .map(size -> {
                    // Obtener precio específico para este tamaño desde product_size_pricing
                    java.math.BigDecimal sizePriceBD = productRepository.findPriceByProductAndSize(product.getId(), size.getId());
                    Double sizePrice;
                    if (sizePriceBD != null) {
                        sizePrice = sizePriceBD.doubleValue();
                    } else {
                        sizePrice = product.getPrice().doubleValue();
                    }
                    return new SizeDTO(
                            size.getId(),
                            size.getName(),
